...
export default function GitHubDashboard() {
  ...
  return (
    <section id="dashboard" ref={ref} className="border-t border-border">
      <div className="section-wrap">
        <p className="mb-2 font-mono text-[11px] text-muted">03 github</p>
        <h2 className="mb-8 text-3xl font-semibold tracking-tight">Activity Dashboard</h2>
          ...
          <div className="space-y-3">
            <motion.div className="grid grid-cols-2 gap-3 md:grid-cols-4" {...up(0.05)}>
              {statCards.map((card) => (
                ...
                >
                  <p className="mb-1 font-mono text-[10px] text-muted">{card.label}</p>
            ...
            <motion.div className="rounded-lg border border-border bg-surface p-5" {...up(0.1)}>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <motion.div className="rounded-lg border border-border bg-surface p-5" {...up(0.15)}>
                <p className="mb-4 font-mono text-[11px] text-muted">monthly commits · 2026</p>
              ...
              <motion.div className="rounded-lg border border-border bg-surface p-5" {...up(0.2)}>
                <p className="mb-4 font-mono text-[11px] text-muted">weekly commit distribution</p>
                  ...
                  {weeklyData.map((item) => {
                    ...
                    return (
                      <div key={item.day} className="group flex flex-1 flex-col items-center">
                        <div className="mb-2 text-[10px] font-mono text-muted">{item.commits}</div>
                        <div className="relative flex h-[120px] w-full items-end justify-center rounded bg-subtle">
                          <motion.div
                        ...
                        <div className="mt-2 font-mono text-[10px] text-muted">{item.day}</div>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <motion.div className="rounded-lg border border-border bg-surface p-5" {...up(0.25)}>
                <p className="mb-4 font-mono text-[11px] text-muted">top languages</p>
                <div className="space-y-3">
                  {data.languages.map((lang, i) => {
                    ...
                    return (
                      <div key={lang.lang}>
                        ...
                        <div className="h-[2px] overflow-hidden rounded-full bg-subtle">
                          <motion.div
              ...
              <motion.div className="rounded-lg border border-border bg-surface p-5" {...up(0.3)}>
                <p className="mb-4 font-mono text-[11px] text-muted">top repositories</p>
                <div className="space-y-2">
                  {data.topRepos.map((repo) => (
                    <a
                      ...
                      className="group flex items-center justify-between rounded border border-border bg-subtle px-3 py-2.5 transition-all hover:border-soft"
                    >
                      <span className="truncate font-mono text-xs text-white transition-colors group-hover:text-soft">
                        {repo.name}
                      </span>
                      <span className="ml-3 shrink-0 font-mono text-[11px] text-muted">
                        ⭐ {repo.stars}
                      </span>
                    </a>
                  ))}
            ...
            <p className="font-mono text-[10px] text-muted">
              source: github public api {data.usesToken ? "(token enabled)" : "(no token)"}
            </p>
          </div>
        )}
      </div>
